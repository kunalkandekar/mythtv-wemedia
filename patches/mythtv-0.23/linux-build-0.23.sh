...
if [ "`grep -c -e release-0-23 version.pro`" -eq "0" ];
then
    echo 'Incorrect version of MythTV. This patch has only been tested with release 0.23'
    exit
fi

# check if patch file exists, apply patch if it does, rename diff file 
# if successfully patched
test -e upnp-wemedia-patch-0.23.diff && echo 'Applying patch...' \
&& patch -p0 -i ./upnp-wemedia-patch-0.23.diff \
&& mv ./upnp-wemedia-patch-0.23.diff ./upnp-wemedia-patched-0.23.diff

test ! -e upnp-wemedia-patched-0.23.diff && echo 'Patch could not be applied successfully... Quitting build.' && exit
